...
const Database = class {
  constructor(collections = {}, storageDir = `${__dirname}/.local`) {
    ...
    const hookDefault = (database, dataBefore) =>
      (database, dataAfter) => null;
  ...
  transactionsCommit() {
    ...
    if (this.__transactionsDepth === 0) {
      ...
      const transactionsFileName = `${this
        .__storageDir}/transactions/${date}.txt`;
      fs.appendFileSync(
  ...
  search(collection, query = {}) {
    ...
    const values = Object.values(this.__data[collection]).filter(i =>
      keys.reduce((a, c) => a && i[c] === query[c], true)
    );
    return values;
  }
  ...
  __restore() {
    ...
    const collectionsContent = fs.readFileSync(
      collectionsFileName,
      { encoding: "utf8" }
    );
